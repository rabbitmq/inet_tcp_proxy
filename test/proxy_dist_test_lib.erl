%% Copied from Erlang/OTP master branch
%% commit: 7ed659fd1144b455883eef18dab38cddc904a065
...
-export([apply_on_test_node/4, apply_on_test_node/2]).
-export([stop_test_node/1, start_test_node/2]).
%%
...
apply_on_test_node(
  ...
  M, F, A) when is_atom(M), is_atom(F), is_list(A) ->
    ...
    apply_on_test_node(Node, Ref, {apply, self(), Ref, M, F, A}).

apply_on_test_node(
  #node_handle{connection_handler = Hndlr} = Node,
  Fun) when is_function(Fun, 0) ->
    Ref = erlang:monitor(process, Hndlr),
    apply_on_test_node(Node, Ref, {apply, self(), Ref, Fun}).

apply_on_test_node(Node, Ref, Msg) ->
    send_to_test_node(Node, Msg),
    receive
...
stop_test_node(#node_handle{connection_handler = Handler,
	...
	ok ->
		...
		{'DOWN', Mon, process, Handler, Reason} ->
			...
			_ ->
			    ct:pal(
                              "stop_test_node/1 ~s Down  ~p ~n",
                              [Name,Reason])
		    end
	    end;
	Error ->
	    erlang:demonitor(Mon, [flush]),
	    ct:pal("stop_test_node/1 ~s Warning ~p ~n", [Name,Error])
    end.

start_test_node(Name, Args) ->
...
mk_node_cmdline(ListenPort, Name, Args) ->
    ...
    ThisTestPa = filename:dirname(code:which(?MODULE)),
    ThisProjectPa = filename:join([ThisTestPa, "..", "ebin"]),
    DepsPa = filelib:wildcard(
               filename:join(
                 [ThisProjectPa, "..", "deps", "*", "ebin"])),
    Pa = string:join([ThisTestPa, ThisProjectPa | DepsPa], " "),
...
%	++ "-run application start crypto -run application start public_key "
	...
	++ "-kernel logger \"[{handler,default,logger_std_h,#{config=>#{file=>\\\"" ++ Pwd ++ "/logger." ++ Name ++ "\\\"}}}]\" "
...
await_test_node_up(Name, LSock) ->
    case gen_tcp:accept(LSock, ?AWAIT_SSL_NODE_UP_TIMEOUT) of
	{ok, Socket} ->
	    gen_tcp:close(LSock),
	    case gen_tcp:recv(Socket, 0) of
		{ok, Bin} ->
		    check_test_node_up(Socket, Name, Bin);
		{error, closed} ->
		    gen_tcp:close(Socket),
		    exit({lost_connection_with_test_node_before_up, Name})
	    end;
	{error, Error} ->
	    gen_tcp:close(LSock),
...
check_test_node_up(Socket, Name, Bin) ->
	...
	{'EXIT', _} ->
	    gen_tcp:close(Socket),
	    exit({bad_data_received_from_test_node, Name, Bin});
	{test_node_up, NodeName} ->
		...
		NodeName ->
		    ...
		    #node_handle{connection_handler = Pid,
				 ...
				 name = Name};
		_ ->
		    exit({unexpected_test_node_connected, NodeName})
	    end;
	Msg ->
	    exit({unexpected_msg_instead_of_test_node_up, Name, Msg})
    end.

send_to_test_node(#node_handle{connection_handler = Hndlr}, Term) ->
    Hndlr ! {relay_to_test_node, term_to_binary(Term)},
    ok.

tstsrvr_con_loop(Name, Socket, Parent) ->
    ok = inet:setopts(Socket,[{active,once}]),
    receive
	{relay_to_test_node, Data} when is_binary(Data) ->
	    case gen_tcp:send(Socket, Data) of
		ok ->
		    ok;
		_Error ->
		    gen_tcp:close(Socket),
		    exit({failed_to_relay_data_to_test_node, Name, Data})
	    end;
	{tcp, Socket, Bin} ->
		...
		bye ->
		    ...
		    exit(normal);
		Unknown ->
		    exit({unexpected_message_from_test_node, Name, Unknown})
            catch
                error : _ ->
		    gen_tcp:close(Socket),
		    exit({bad_data_received_from_test_node, Name, Bin})
	...
	{tcp_closed, Socket} ->
	    gen_tcp:close(Socket),
	    exit({lost_connection_with_test_node, Name});
        {'EXIT', Parent, Reason} ->
...
notify_test_node_up(Socket) ->
    case catch gen_tcp:send(Socket,
			    term_to_binary({test_node_up, node()})) of
	ok -> ok;
